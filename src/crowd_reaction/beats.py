...
class BEATs(nn.Module):
    def __init__(self, cfg: BEATsConfig) -> None:
        super().__init__()
        logger.info("BEATs Config: %s", cfg.__dict__)
...
class TransformerEncoder(nn.Module):
    ...
    def extract_features(self, x, padding_mask=None, tgt_layer=None):
        ...
        return x.transpose(0, 1), layer_results
...
class TransformerSentenceEncoderLayer(nn.Module):
    def __init__(
        ...
        self.deep_norm_alpha = math.pow(2 * encoder_layers, 1 / 4) if self.deep_norm else 1
...
class MultiheadAttention(nn.Module):
    ...
    def compute_bias(self, query_length, key_length):
        ...
        buckets = self._relative_positions_bucket(relative_position, bidirectional=True)
    ...
    def forward(
        ...
        if position_bias is not None:
            ...
            if self.gru_rel_pos == 1:
                ...
                batch_size, num_heads, seq_len, _ = query_layer.size()
                gate_a, gate_b = torch.sigmoid(
                    self.grep_linear(query_layer).view(batch_size, num_heads, seq_len, 2, 4).sum(-1)
                ).chunk(2, dim=-1)
        ...
        if need_weights:
            attention_weights = attn_weights_float.view(bsz, self.num_heads, tgt_len, src_len).transpose(1, 0)
            if not need_head_weights:
                attention_weights = attention_weights.mean(dim=0)
        return attn, attention_weights, position_bias
    ...
    def _append_prev_key_padding_mask(
        ...
        if prev_key_padding_mask is not None and key_padding_mask is not None:
            return torch.cat([prev_key_padding_mask.float(), key_padding_mask.float()], dim=1)
    ...
    def _get_input_buffer(
        ...
        if incremental_state is None:
            return {}
        return incremental_state.get("attn_state", {})
